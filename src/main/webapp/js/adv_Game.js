...
function InitTable(pageindex) {
    ...
    $.ajax({
        url: getRootPath()+"/game/list/?Id=0&t=" + Math.random(),
        type: 'POST',
        dataType: 'json',
        timeout: 30000,
        data: "pageIndex=" + (pageindex + 1) + "&pageSize=" + pageSize ,// + $("#SouceType").val(),
        error: function () {
        ...
        success: function (data) {
            ...
            for (var n = 0; n < data.rows.length; n++) {
                ...
                content += "<div class=\"file-name\">" + data.rows[n].FileNameStr + "<br/>";
...
function DelGame(state) {
    var Ids = GetGameIds();
    if (isBlank(Ids)) {
        if (state == 0) {
            parent.ShowWarning(106, 10010, "Game", 1)
        }
        else {
            $.post(getRootPath()+"/game/delete/?resIds=" + Ids, function (obj) {
                if (obj.IsOk) {
...
function AddGame() {
    parent.OpenModal(getRootPath()+'/game/addGamePage/', '新增游戏', "750px");
}
