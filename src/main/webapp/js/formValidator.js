    ...
    $.formValidator =
{
    ...
    setTipState: function (elem, showclass, showmsg) {
        ...
        $("#" + elem.id).removeClass("blueInput").removeClass("redInput");
        if ((showmsg == null || showmsg == "") && showclass != "onCorrect") {
            tip.hide();
        }
        else {
            if (showclass == "onError") {
                $("#" + elem.id).addClass("redInput");
            }
            if (showclass == "onFocus") {
                $("#" + elem.id).removeClass("redInput").addClass("blueInput");
            }
            ...
            if (showmsg == "0") {
                ...
                tip.width("auto");
                tip.css("padding", "0 5px 0 39px");
                if (showclass == "onShow") {
                    tip.css("padding-left", "10px");
                }
                tip.html(showmsg);
            }
        }
    ...
    //设置指定的显示信息
    setStateTip: function (tipid, showclass, showmsg) {
        var tip = $("#" + tipid);
        var elemId = $("#" + tipid.substring(0, tipid.lastIndexOf("Tip")));
        elemId.removeClass("blueInput").removeClass("redInput");
        if (showclass == "onError") {
            elemId.addClass("redInput");
        }
        if (showclass == "onFocus") {
            elemId.addClass("blueInput");
        }
        tip.removeClass();
        tip.addClass(showclass);
        if (showmsg == "0") {
    ...
    showMessage: function (returnObj) {
        ...
        if (!isvalid) {
            ...
            //else
            if ($.formValidator.isEmpty(id))
                $.formValidator.setTipState(elem, "onempty", "");
            else
                $.formValidator.setTipState(elem, "onCorrect", setting.oncorrect);
        }
        return showmsg;
    },
    ...
    pageIsValid: function (validatorgroup) {
        ...
        jqObjs.each(function (i, elem) {
            if (elem.settings[0].bind) {
                returnObj = $.formValidator.oneIsValid(elem.id, 2);
                if (returnObj) {
        ...
        if (isvalid) {
            ...
            if (thefirstid != "") { document.getElementById(thefirstid).scrollIntoView(); if (initConfig.errorfocus) { $("#" + thefirstid).focus(); } }
        }
    ...
    ajaxValid: function (returnObj) {
        ...
        if(setting.paramIdGroup.length>0)
        {
            for(var i=0;i< setting.paramIdGroup.length;i++){
                var _id1 = setting.paramIdGroup[i].id;
                var _param1 = setting.paramIdGroup[i].paramName ? setting.paramIdGroup[i].paramName : _id1;
                ls_url = ls_url + (ls_url.indexOf("?") > 0 ? ("&" +_param1 +"="+encodeURIComponent($("#"+_id1).val())) : ("?" + _param1+"="+encodeURIComponent($("#"+_id1).val()))); 
            }
        }
        $.ajax(
		{
