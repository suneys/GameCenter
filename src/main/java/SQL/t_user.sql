...
CREATE TABLE `t_user` (
  ...
  `user_role` tinyint(4) default NULL COMMENT '用户角色，0-普通用户；1-代理商；2-超级用户',
  `create_time` datetime default NULL COMMENT '创建时间',
  `modify_time` datetime default NULL COMMENT '最后修改时间',
  `is_enable` tinyint(4) default NULL COMMENT '是否可用，0-不可用；1-可用',
  `is_delete` tinyint(4) default NULL COMMENT '是否删除，0-未删除；1-已删除',
  `user_DaySend` int(11) default NULL COMMENT '当天发送的数量',
  `user_TotalSend` int(11) default NULL COMMENT '总共发送的数量',
  PRIMARY KEY  (`uId`)
